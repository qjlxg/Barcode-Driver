...
async def main():
    ...
    total = 0
    for l in lines:
        total += len(PATHS) if ":" in l else len(TARGET_PORTS) * len(PATHS)

    queue: asyncio.Queue = asyncio.Queue(maxsize=8000)
    write_queue: asyncio.Queue = asyncio.Queue()

    async with aiohttp.ClientSession(
        connector=aiohttp.TCPConnector(ssl=False, limit=WORKER_COUNT*2, ttl_dns_cache=300, force_close=True),
        ...
        for item in lines:
            if ":" in item:
                ...
                for path in PATHS: await queue.put((host, int(port_str), path))
            else:
