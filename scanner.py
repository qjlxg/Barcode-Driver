...
async def writer_worker(write_queue: asyncio.Queue):
    ...
    with open(csv_path, 'a', newline='', encoding='utf-8') as csvfile:
        ...
        if os.path.getsize(csv_path) == 0: writer.writerow(['hash', 'url', 'type'])
        while True:
...
async def scanner_worker(queue: asyncio.Queue, write_queue: asyncio.Queue, session: aiohttp.ClientSession):
    while True:
        ...
        try:
            async with session.get(url, headers={"User-Agent": random.choice(UA_LIST)}, timeout=aiohttp.ClientTimeout(total=REQUEST_TIMEOUT), ssl=False, allow_redirects=True) as resp:
                stats["req"] += 1
                if resp.status == 200:
                    ...
                    # --- 强制即时刷新输出，确保日志可见 ---
                    sys.stdout.write(f"\n[DEBUG] 尝试URL: {url}\n")
                    sys.stdout.write(f"[DEBUG] 内容长度: {len(text)}\n")
                    sys.stdout.write(f"[DEBUG] 预览前100字符: {text[:100].replace(chr(10), ' ')}\n")
                    sys.stdout.flush() 
                    # -----------------------------------
...
async def main():
    ...
    async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=False, limit=WORKER_COUNT*2), headers={"Connection": "close"}) as session:
